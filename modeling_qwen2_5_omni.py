...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        self.system_cache_num = -1
        self.is_first_gene = True
        self.llm_past_key_values = None
        self.llm_past_key_values_for_system = {}
        self.post_init()
    ...
    def streaming_prefill_system(
        ...
        self.llm_past_key_values_for_system[self.system_cache_num] = outputs["past_key_values"]
        #### carl add 12.24
    ...
    def streaming_prefill(
        ...
        if self.system_cache_num >= 0:
            cache_length_system = self.llm_past_key_values_for_system[self.system_cache_num][0][0].shape[2]
        

        if self.llm_past_key_values is not None:
            cache_length = self.llm_past_key_values[0][0].shape[2] +  cache_length_system
        else:
            cache_length = 0 +  cache_length_system
            self.llm_past_key_values = self.llm_past_key_values_for_system[self.system_cache_num]
    ...
    def forward(
        ...
        if self.use_stream:
            outputs = self.model(
                    ...
                    ### carl add  for new ...no stream
            outputs = self.model(
                    attention_mask=attention_mask,
                    position_ids=position_ids,
                    #position_ids=None,
                    past_key_values=self.llm_past_key_values_for_system[self.system_cache_num],            
                    inputs_embeds=inputs_embeds,
                    use_cache=use_cache,
                    output_attentions=output_attentions,
                    output_hidden_states=output_hidden_states,
...
class Qwen2_5OmniForConditionalGeneration(Qwen2_5OmniPreTrainedModel, GenerationMixin):
    ...
    def generate(
        ...
        **kwargs,
        ...
        self.system = use_system
        self.system_cache_num = system_cache_num
        self.end = is_end
        ...
        self.thinker.system_cache_num = self.system_cache_num
        ### carl add 1223...
        if self.system:
            thinker_result = self.thinker.streaming_prefill_system(input_ids=input_ids,**thinker_kwargs)
            print("release all variable for system prefilling .......... ....")
            self.thinker.use_stream = False
            self.thinker.is_first_gene = True
            self.thinker.llm_past_key_values = None
            return None
        ...
        print("release all variable for new turn audio ....")        
        self.thinker.use_stream = False     
        self.thinker.is_first_gene = True     
        self.thinker.llm_past_key_values = None      
